...
function SkinDiseaseDetection() {
  return <DetectionTool title="Skin Disease Detection" apiUrl="https://detect.roboflow.com/skin-disease-ieqns/3" />;
}
...
function DetectionTool({ title, apiUrl }) {
  ...
  const displayWidth = 320; // Fixed display width for image scaling
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        if (predictions.length > 0) {
          ...
          setMessage(
            `The image shows ${Math.round(highestConfidenceBox.confidence * 100)}% probability of having ${highestConfidenceBox.class}.`
          );
        } else {
          setMessage("No skin disease detected.");
        }
      } catch (error) {
  ...
  return (
    <div className="flex flex-col items-center p-6 bg-gray-100 min-h-screen">
      ...
      <label className="cursor-pointer bg-yellow-500 text-white px-4 py-2 rounded-lg shadow-md hover:bg-yellow-600 transition">
        Upload Image
            ...
            return (
              <div key={index}>
                ...
                <div
                  ...
                  style={{
                    left: `${(box.x - box.width / 2) * scaleFactor}px`,
                    top: `${(box.y - box.height / 2) * scaleFactor - 22}px`, // Moves text above the box
                  }}
                >
                ...
                <div
                  ...
                  style={{
                    left: `${(box.x - box.width / 2) * scaleFactor}px`,
                    top: `${(box.y - box.height / 2) * scaleFactor}px`,
